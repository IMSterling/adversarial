...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, multi_label=True, classes=None, agnostic=False):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Compute conf
        confidences = x[..., 4:5].clone()
        original_values = x[..., 5:].clone()
        x[..., 5:] = original_values * confidences  ##THIS IS THE PROBLEMATIC LINE OF CODE
        ...
        if merge and (1 < n < 3E3):  # Merge NMS (boxes merged using weighted mean)
            try:  # update boxes as boxes(i,4) = weights(i,n) * boxes(n,4)
                ...
                ####TEMPORARAY NEXT LINES
                temp = x[:, :4].clone()
                temp4 = torch.mm(weights, temp).float()
                temp5 = weights.sum(1, keepdim=True)
                temp1 = temp4/temp5
                x[i, :4] = temp1

                ###########
                #x[i, :4] = torch.mm(weights, x[:, :4]).float() / weights.sum(1, keepdim=True) #THIS IS THE ORIGINAL LINE OF CODE # merged boxes
