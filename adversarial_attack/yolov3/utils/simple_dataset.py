...
class LoadImages:  # for inference
    def __init__(self, image_tensor, img_size=416):
        tensor = image_tensor # os-agnostic
        files = []
        self.cap = None
        for i in range(tensor.shape[0]): ##potentially the issue
            single_mat = tensor[i,:,:,:]
    ...
    def __next__(self):
        ...
        # Read image
        self.count += 1
        scale_factor = self.img_size/img0.shape[1]
        newy = int(np.round(img0.shape[0]*scale_factor,0))
        newx = self.img_size
        img = img0
        ...
        # Padded resize
        #img = letterbox(img0, new_shape=self.img_size)[0] #THIS IS WHERE THE PROBLEM IS
        #img = nn.upsample(newy,newx)
        #img = tf.image.resize_with_pad(img0,newy,newx,method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)

        # Convert
        #img = img[:, :, ::-1]#BGR to RGB
       # img = img.permute(2,0,1) JUST COMMENTED THIS OUT
        img = img[np.newaxis,:,:,:]
        img = img.type(torch.float)
        img = interpolate(img,size=(384,512),mode='nearest') #scale_factor=scale_factor)
        img=img[0,:,:,:]#IF this does not work first change height and width
